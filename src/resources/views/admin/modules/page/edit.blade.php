        ...
        <div class="admin_edit_block">
            ...
            <form  method="post" enctype="multipart/form-data" class="admin_edit-form">
                ...
                @include('admin.includes.select', [
                    ...
                    'select'=> \App\Models\Pages\Pages::where('id', '<>', $object->id)->get()
                ])
                ...
                {!! \App\Helpers\GenerateForm::makeImage('Баннер', 'image' , $object , '/storage/'.$object->image) !!}
                ...
                @switch ($object->page_code)
                    ...
                    @break
                    @case('main')
                        @include('admin.includes.input', ['label' => 'День открытых дверей (дата):', 'name' => 'text_2', 'value' => $object->text_2 ?? ''])
                        @include('admin.includes.input', ['label' => 'День открытых дверей (время):', 'name' => 'text_3', 'value' => $object->text_3 ?? ''])
                    @break
                    @case('aspirantura')
                        @include('admin.includes.input', ['label' => 'Телефон:', 'name' => 'text_2', 'value' => $object->text_2 ?? ''])
                        @include('admin.includes.input', ['label' => 'Email:', 'name' => 'text_3', 'value' => $object->text_3 ?? ''])
                    @break
                @endswitch
                ...
                <x-Admin.Accordion title="Английская версия">
                    @include('admin.includes.input', ['label' => 'Заголовок', 'name' => 'name_en', 'value' => $object->name_en ?? ''])
                            @switch ($object->page_code)
                                @case('istoriia')

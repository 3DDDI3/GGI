                    ...
                    <div class='search-select3'>
                        ...
                        <select name='select3' class='chosen'>
                            ...
                            @foreach ($select3 AS $option)
                                <option value='{{ $option->id ?? 0 }}' {{ ($_GET['select3'] ?? '') == $option->id ? 'selected' : '' }}>{{ $option->name ?? '' }}</option>
                            @endforeach
                        </select>
                    </div>
                @endif
                    ...
                    <div class='search-select4'>
                        ...
                        <select name='select4' class='chosen'>
                            ...
                            @foreach ($select4 AS $option)
                                <option value='{{ $option->id ?? 0 }}' {{ ($_GET['select4'] ?? '') == $option->id ? 'selected' : '' }}>{{ $option->name ?? '' }}</option>
                            @endforeach
                        </select>
                    </div>
                @endif
                ...
                <div class='search-select'>
                    ...
                    <select name='select' class='chosen'>
                        ...
                        @foreach ($select AS $option)
                            <option value='{{ $option->id ?? 0 }}' {{ ($_GET['select'] ?? '') == $option->id ? 'selected' : '' }}>{{ $option->name ?? '' }}</option>
                        @endforeach
                    </select>
                </div>
            @endif
                ...
                <div class='search-select2'>
                    ...
                    <select name='select2' class='chosen'>
                        ...
                        @foreach ($select2 AS $option)
                            <option value='{{ $option->id ?? 0 }}' {{ ($_GET['select2'] ?? '') == $option->id ? 'selected' : '' }}>{{ $option->name ?? '' }}</option>
                        @endforeach
                    </select>
                </div>
            @endif

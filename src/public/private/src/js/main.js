...
$(document).ready(function () {
    ...
    $("input[name='phone']").not('.select-phone').inputmask('+7 (999) 999-99-99');
    ...
    $(".add-new-work__button").on("click", function (e) {
        e.preventDefault();

        let newBlock = $(".work").clone();
        let year = $("select[name='workYear']").val();

        $($(newBlock).find("legend")[0]).html($($(newBlock).find("legend")[0]).html().replace(/Работы года/, `Работы ${year} года`));
        $($(newBlock).find("legend")[0]).parents(".work").find("input[name='year']").val(year);

        Array.from($($(newBlock).find("legend")[0]).parents(".work").find("input")).forEach(el => {
            // $(el).attr("name", `${$(el).attr("name")}-${$("select[name='workYear']").val()}`);
            let matches = $(el).attr("name").match(/(individualPlan)-(\d{1,2})|(review)-(\d{1,})|(yearPlan)-(\d{1,2})|(extract)-(\d{1,2})|(protocol)-(\d{1,2})|(workReport)-(\d{1,2})/);

            if (matches != null) {
                matches = matches.filter(Boolean);
                $(el).attr("name", `${matches[1]}-${year}-${matches[2]}`);
            }
        });

        $(".works").prepend(newBlock);

        $(".works .work:first").css("display", "block");
        $(".works .work:first .work__wrapper").show("slow");
        $(".works .work:first .work__wrapper").parents(".work").find("legend svg").css("transform", "rotate(180deg)");
    });

    $(".works").on("click", ".work legend", function () {
        if ($(this).parents(".work").find(".work__wrapper").css("display") == "block") {
            $(this).parents(".work").find(".work__wrapper").hide("slow");
            $(this).parents(".work").find("legend svg").css("transform", "rotate(0deg)");
        }
        else {
            $(this).parents(".work").find(".work__wrapper").show("slow");
            $(this).parents(".work").find("legend svg").css("transform", "rotate(180deg)");
        }
    });

    // $(".works").on("click", ".work__wrapper div button", function (e) {
    //     e.preventDefault();

    //     $(this).parents(".work").hide(500);
    //     setTimeout(() => {
    ...
    $(".works").on("click", ".work__wrapper .work__save", function (e) {
        e.preventDefault();
        console.log($(this).parents(".work__wrapper").find("fieldset.documents input[type='file']"));

        let formData = new FormData();

        let types = new Map([
            ["workReport", "Реферат"],
            ["individualPlan", "Индивидуальный план научной деятельности по годам/семестрам"],
            ["yearPlan", "План научной деятельности по годам"],
            ["review", "Отзыв научного руководителя"],
            ["extract", "Выписка из протокола семинара"],
            ["protocol", "Протокол отчета на Ученом совете"],
        ])

        Array.from($(this).parents(".work__wrapper").find("fieldset.documents input[type='file']")).forEach(el => {
            let matches = $(el).attr("name").match(/(individualPlan)-(\d{4})-(\d{1,2})|(review)-(\d{4})-(\d{1,2})|(yearPlan)-(\d{4})-(\d{1,2})|(extract)-(\d{4})-(\d{1,2})|(protocol)-(\d{4})-(\d{1,2})|(workReport)-(\d{4})-(\d{1,2})/);
            ...
            if (matches != null) {
                ...
                if (el.files.length > 0) {
                    ...
                    formData.append("document", types.get(matches[1]));
                    formData.append("year", matches[2]);
                    formData.append("page", "Персональные данные");
                    formData.append("user", location.pathname.match(/\d+$/)[0]);

                    axios.post("/api/pa/users/files/upload", formData)
                        .then(response => {
                            Swal.fire({
                                title: "Файлы успешно сохранены",
                                icon: 'success',
...
if ($('.chosen').length > 0) {
    ...
    $('.search-select .chosen, .search-select2 .chosen, .search-select3 .chosen, .search-select4 .chosen').change(function () {
...
$('.js-bonus-accrual__button').on('click', function () {
    ...
    if (type == 'review') {
        ...
        parent.html("<strong>За отзыв начислено <span>" + count + "</span> бонусов</strong>")
    }

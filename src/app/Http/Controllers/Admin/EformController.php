...
class EformController extends Controller {
  public $PATH = 'eform';
  public $TITLE = ['Электронная форма обращений', 'раздела'];
  ...
  public function index( Request $request ) {
    ...
    if ($request->search)
      $objects = Helper::search($objects, $request->search, ['name']);
    ...
    foreach ($objects as $key => &$obj) {
      $obj->file = Files::getFile('eform', $obj->id);
      // dd($objects);
      if (!$obj->file->isEmpty()){
        $obj->file = $obj->file[0]->path;}else{
          $obj->file = "";
        }
    }
    // dd($objects);
  ...
  public function edit( Request $request, $id = null ) {
    ...
    if ($request->isMethod('post')) {
      ...
      // FileUpload::uploadFile('path', Documents::class, 'path', $object->id, '/files/council');
